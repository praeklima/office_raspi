...
while True:
    ...
    if 700 <= int(current_time) <= 1800:
        ...
        sys.stdout.write("\r")
        print("release time of CO2 and Humidifier is ", 5 * no_of_users, "seconds", " Valve 1 = ON and Valve 2 = ON", end='', flush=True)
        time.sleep(5 * no_of_users)
        openhab_data_access.openhab_send_command(localhost_url, 'Socket_Switch_4', 'OFF')
        sys.stdout.write("\r")
        print("release time of CO2 and Humidifier is ", 5 * no_of_users, "seconds", " Valve 1 = ON and Valve 2 = OFF", end='', flush=True)
